...
use crate::{Ed25519KeyPair, TonClient};
mod test_piggy;
...
fn test_contracts() {
    ...
    let keys: Ed25519KeyPair = serde_json::from_str(
        r#"{
    ...
    let prepared_wallet_address = ton
        .contracts
        .get_deploy_address(&base64::decode(WALLET_CODE_BASE64).unwrap(), &keys)
        .unwrap();
